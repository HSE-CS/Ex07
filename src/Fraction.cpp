...
int nod_func(int x, int y) {
    ...
    if (x == y) // если числа равны, НОД найден
    return x;
    ...
    if (x < y)
    { 
      d = nod_func(x, r); // вызываем функцию nod_func() для двух наименьших чисел
    }
    else
    {
      d = nod_func(y, r); // вызываем функцию nod_func() для двух наименьших чисел
    }
    return d;
}
